...
static void registerOrderInteractively(LivroOrdensAVL& orderBook) {
...
static void cancelOrderInteractively(LivroOrdensAVL& orderBook) {
...
static void executeOrderInteractively(LivroOrdensAVL& orderBook) {
...
static void displayOperationsHistory(LivroOrdensAVL& orderBook) {
...
static void searchOrderInteractively(LivroOrdensAVL& orderBook) {
...
static void listAllActiveOrders(LivroOrdensAVL& orderBook) {
...
static void showTreeStatistics(LivroOrdensAVL& orderBook) {
    std::cout << "\n----- Estatisticas da Arvore AVL -----\n";
    std::cout << "Quantidade de ordens (nos): " << orderBook.quantidadeOrdens() << "\n";
    std::cout << "Altura da arvore:           " << orderBook.alturaArvore() << "\n";
    std::cout << "Rotacoes LL (insercao):     " << orderBook.totalRotacoesSimplesEsquerdaEsquerda() << "\n";
    std::cout << "Rotacoes LR (insercao):     " << orderBook.totalRotacoesDuplasEsquerdaDireita() << "\n";
    std::cout << "Rotacoes RL (insercao):     " << orderBook.totalRotacoesDuplasDireitaEsquerda() << "\n";
    std::cout << "Rotacoes RR (insercao):     " << orderBook.totalRotacoesSimplesDireitaDireita() << "\n";
    std::cout << "Rotacoes LL (remocao):      " << orderBook.totalRotacoesSimplesEsquerdaRemocao() << "\n";
    std::cout << "Rotacoes LR (remocao):      " << orderBook.totalRotacoesDuplasEsquerdaRemocao() << "\n";
    std::cout << "Rotacoes RL (remocao):      " << orderBook.totalRotacoesDuplasDireitaRemocao() << "\n";
    std::cout << "Rotacoes RR (remocao):      " << orderBook.totalRotacoesSimplesDireitaRemocao() << "\n";
    std::cout << "Total de rotacoes:          " << orderBook.totalRotacoes() << "\n";
    std::cout << "Bytes estimados por no:     " << LivroOrdensAVL::bytesPorNoEstimado() << " bytes\n";
    std::cout << "Memoria total estimada:     " << orderBook.bytesTotaisEstimados() << " bytes\n";
    std::cout << "---------------------------------------\n";
}

static void countOrdersByTickerInteractively(LivroOrdensAVL& orderBook) {
...
static void listOrderCountByTickerInteractively(LivroOrdensAVL& orderBook) {
...
static void insertRandomOrdersForBenchmark(LivroOrdensAVL& orderBook, long ordersCount, std::mt19937& rngEngine, std::vector<ChaveOrdem>& insertedKeys) {
...
static void runComparativeBenchmark() {
    ...
    std::ofstream csvFile("resultados_avl.csv");
    csvFile << "arvore,n,tempo_insercao_ms,altura_apos_insercao,rotacoes_insercao,"
            << "tempo_busca_ms,tempo_remocao_ms,altura_apos_remocao,rotacoes_remocao,memoria_bytes\n";
    ...
    for (long n : sizeList) {
        ...
        csvFile << "AVL," << n << "," << durationInsertMs << "," << heightAfterInsert << ","
                << rotationsInsert << "," << durationSearchMs << "," << durationRemoveMs << ","
                << heightAfterRemove << "," << rotationsRemove << "," << orderBook.bytesTotaisEstimados() << "\n";
    }

    csvFile.close();
    std::cout << "\nResultados exportados para 'resultados_avl.csv'.\n";
    std::cout << "(quando a Rubro-Negra estiver pronta, gerem o mesmo CSV\n"
              << " com arvore=\"RN\" e comparem as colunas nos graficos)\n";
    std::cout << "===========================================\n";
}

static void displayMenuOptions() {
    std::cout << "\n===== LIVRO DE ORDENS - ARVORE AVL =====\n"
              ...
              << "12 - Benchmark comparativo em varios tamanhos (gera resultados_avl.csv)\n"
              << "0  - Sair\n"
              << "Escolha uma opcao: ";
}

int main() {
    LivroOrdensAVL internalOrderBook;
    int selectionIndex = -1;

    std::cout << "Bem-vindo ao sistema simplificado de Livro de Ordens.\n"
              << "Este programa demonstra o uso de uma Arvore AVL para\n"
              << "gerenciar ordens de compra e venda de acoes.\n";
